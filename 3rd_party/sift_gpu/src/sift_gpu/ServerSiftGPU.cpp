...
public:
	...
	static int init()
	{
		...
		{
			std::cout << "server: can't startup socket" << std::endl;
			return 0;
		}else
        {
            return 1;
...
int ServerSiftGPU::StartServerProcess(int argc, char** argv)
{
	...
	///////////////////////////////////////////////////////
	//make a new process
  #ifndef SIFT_GPU_BIN_RELEASE_FOLDER
    #define SIFT_GPU_BIN_RELEASE_FOLDER ""
  #endif
#ifdef _WIN32
    int result = (int) _spawnv(_P_NOWAIT, SIFT_GPU_BIN_RELEASE_FOLDER "server_siftgpu.exe", &args[0]);
    if (result == -1)
    {
      std::cerr << "spawn returns -1 with error = " << errno << std::endl;
      std::cerr << "Attempting to run: " << SIFT_GPU_BIN_RELEASE_FOLDER "server_siftgpu.exe" << std::endl;
    }
	return  result != -1;
#else
	#ifdef __APPLE__
    ...
    if(oldpath == NULL)
    {
	    result =  posix_spawn(&pid, SIFT_GPU_BIN_RELEASE_FOLDER "server_siftgpu", NULL, NULL, &args[0], NULL);
    }else
    {
        char newpath[1024]= LIBPATH_NAME "=";
        strcat(newpath, oldpath);
        char* envp [] = {newpath, 0};
        result = posix_spawn(&pid, SIFT_GPU_BIN_RELEASE_FOLDER "server_siftgpu", NULL, NULL, &args[0], envp);
    }
	if(result) std::cerr << "failed to use poxis_spawn to create the server." << std::endl;
	return  result == 0;
#endif
}
...
inline void ServerSiftGPU::ServerLoop(int port, int argc, char** argv)
{
    ...
    {
	    std::cout << "server: can't bind to port " <<  port << std::endl;
	    return;
		    ...
		    case COMMAND_RUNSIFT_DATA:
			    {
					...
					//std::cout << "RunSIFT: [" << data_des[0] << "x" << data_des[1] << "]"; // commented out by jheinly
...
int ServerSiftGPU:: InitializeConnection(int argc, char** argv)
{
    ...
    if(_server_name[0] == 0)
    {
	    ...
	    {
		    std::cout<<"Unable to start local siftgpu server" << std::endl;
            return 0 ;
	    }
	...
	if(ConnectServer(server_name, _port) == 0)
	{
		...
		if(ConnectServer(server_name, _port) == 0)
		{
			std::cout<<"Unable to connect siftgpu sever" << std::endl;
			return 0 ;
		}
	}
	return 1;
...
int ServerSiftGPU::VerifyContextGL()
{
	...
	{
		std::cout<<"SifGPU failed to initialize" << std::endl;
		Disconnect();
		return 0;
	}
}
...
ComboSiftGPU* CreateRemoteSiftGPU(int port, char* remote_server)
{
    std::cout << "ServerSiftGPU need macro SERVER_SIFTGPU_ENABLED." << std::endl
              << "Use local SiftGPU/SiftMatchGPU instead." << std::endl;
